...
then
  ...
  then
    echo "Command: ${SCRIPT_NAME} ${*}"
    echo "Usage: ${SCRIPT_NAME} <volume name>"
    echo "RuntimeAbortWorkflow: \"${VOLUME_PATH}/etc/deploystudio/bin/ds_active_directory_binding.mobileconfig\" configuration profile not found!"
    exit 1
  else
    rm "${VOLUME_PATH}"/etc/deploystudio/bin/ds_active_directory_binding.plist 2>/dev/null
    ...
    cp "${SCRIPT_PATH}"/ds_active_directory_binding/ds_active_directory_binding.10.11.sh "${VOLUME_PATH}"/etc/deploystudio/bin/ds_active_directory_binding.sh
  fi
else
  ...
  then
    ...
    rm "${VOLUME_PATH}"/etc/deploystudio/bin/ds_active_directory_binding.mobileconfig 2>/dev/null
