...
if [ -n "${COMPRESSED}" ]
then
  gunzip -c "${IMAGE_FILE}" | "${TOOLS_FOLDER}"/ntfsclone --restore-image --overwrite "${NTFS_DEVICE}" -
else
  "${TOOLS_FOLDER}"/ntfsclone --restore-image --overwrite "${NTFS_DEVICE}" "${IMAGE_FILE}"
fi
NTFSCLONE_RC=${?}
...
then
  ...
  echo "${TOOLS_FOLDER}"/ntfsresize -f "${NTFS_DEVICE}"
  "${TOOLS_FOLDER}"/ntfsresize -f "${NTFS_DEVICE}"
  if [ ${?} -ne 0 ]
  then
